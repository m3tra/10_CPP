...
Form::Form() : _name("no_name"), _isSigned(false), _gradeToSign(0), _gradeToExec(0) {}
...
Form::NotSignedException::NotSignedException(const char* context) : _context(context) {}
...
void			Form::execute(Bureaucrat const & executor) const throw (GradeTooLowException, NotSignedException) {
	if (!this->isSigned())
		throw (NotSignedException("Form isn't signed"));
